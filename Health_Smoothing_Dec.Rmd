...
```{r}
environ <- st_read("/Users/karidachai/Desktop/Fall 2021/MA675/tree/Tree_dem__env_1129/Tree_dem_env.shp",quiet = TRUE)
names(environ)
names(tract2)
...
```{r}
rbind(ks_as$prior_coef, ks_as$prior_coef_se)
```

```{r}
tract7 <- st_read("/Users/karidachai/Desktop/Fall 2021/MA675/tree/matches/matches.shp", quiet = T)
tract7$ks_as[tract7$GEOID20 %in% newhealth$GEOID20] <- ks_as$smooth[newhealth$GEOID20 %in% tract7$GEOID20]
ggplot() + geom_sf(color = 'grey', data = tract7) + geom_sf(aes(fill = ks_as), data=na.omit(tract7)) + 
...
par(op)
rbind(ks_mh$prior_coef, ks_mh$prior_coef_se) #to see coef values
```


```{r}
tract7$ks_mh[tract7$GEOID20 %in% newhealth$GEOID20] <- ks_mh$smooth[newhealth$GEOID20 %in% tract7$GEOID20]
