...
tm_shape(tract) +
    tm_polygons(col ="Mental",palette = "Reds",style = "fixed", breaks = seq(2.2,2.6, 0.1),
                legend.hist = TRUE) +
    tm_layout(legend.outside = TRUE)
...
y <- newhealth$smoothb
op <- par(mfrow = c(1, 2))
plot(ks_mh$smooth, y, ylim = c(0.8, 1.1)); abline(0, 1, col = "red") 
...
```{r}
coef2$Q97.5 <- coef2$V1 + 2 * coef2$V2
coef2$Q2.5 <- coef2$V1 - 2 * coef2$V2
